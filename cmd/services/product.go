...
func (p *ProductService) GetProducts(ctx context.Context, pageParam *productPb.Page) (*productPb.Products, error){
	...
	postgres := p.DB.Table("products AS p").
	...
	//panggil pagination
	offset, limit := helpers.Pagination(postgres, page, &pagination)
	rows , err := postgres.Offset(int(offset)).Limit(int(limit)).Rows()
...
func (p *ProductService) CreateProduct(ctx context.Context, productData *productPb.Product)(*productPb.Id, error){
	...
	err := p.DB.Transaction(func(tx *gorm.DB) error{
		...
		//cek nama kategori udh ada atau belum, kalo udah ada namanya diambil, kalo belum namanya dibikin trus diambil juga
